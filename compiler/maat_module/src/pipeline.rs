...
//! gets its own [`TypeEnv`](maat_types::TypeEnv) with imported bindings), then compiles all
...
use crate::{
    ModuleExports, ModuleGraph, ModuleId, ModuleResult, ResolvedImport, module_exports,
    module_imports,
};
...
fn type_check_modules(
    ...
    for &module_id in topo_order {
        ...
        for import in &imports {
            module_imports::inject_into_env(checker.env_mut(), import);
        }
        ...
        let module_exports = module_exports::extract(program, checker.env());
...
fn compile_modules(
    ...
    for &module_id in topo_order {
        ...
        if let Some(imports) = cached_imports.get(&module_id) {
            for import in imports {
                module_imports::inject_into_compiler(&mut compiler, import);
            }
        }
...
fn resolve_imports(
    ...
    for stmt in &program.statements {
        ...
        for item_name in &items_to_import {
            module_exports::find(target_exports, item_name, &mut result);
        }
    }
