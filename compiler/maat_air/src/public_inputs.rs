...
impl ToElements<BaseElement> for MaatPublicInputs {
    fn to_elements(&self) -> Vec<BaseElement> {
        let mut elements = Vec::with_capacity(
            self.inputs.len() + 4 + self.output_segment.len() + self.program_segment.len(),
        );
...
mod tests {
    ...
    fn to_elements_includes_inputs_output_and_bases() {
        ...
        // 2 inputs + 1 output + 1 output_base + 0 output cells + 1 program_base + 0 program cells.
        assert_eq!(elements.len(), 5);
        assert_eq!(elements[0], BaseElement::new(10));
        assert_eq!(elements[2], BaseElement::new(42));
        assert_eq!(elements[3], BaseElement::ZERO);
        assert_eq!(elements[4], BaseElement::ZERO);
    }
    ...
    fn to_elements_includes_output_and_program_segments() {
        ...
        let elements = pi.to_elements();
        // 0 inputs + 1 output + 1 output_base + 3 output cells + 1 program_base + 2 program cells.
        assert_eq!(elements.len(), 8);
        assert_eq!(elements[0], BaseElement::new(100));
        assert_eq!(elements[1], BaseElement::new(17));
        assert_eq!(elements[2], BaseElement::new(7));
        assert_eq!(elements[3], BaseElement::new(13));
        assert_eq!(elements[4], BaseElement::new(31));
        assert_eq!(elements[5], BaseElement::ONE);
        assert_eq!(elements[6], BaseElement::new(0xAA));
        assert_eq!(elements[7], BaseElement::new(0xBB));
    }
    ...
    fn with_output_has_empty_inputs_and_segments() {
        ...
        let elements = pi.to_elements();
        // 1 output + 1 output_base + 1 program_base.
        assert_eq!(elements.len(), 3);
        assert_eq!(elements[0], BaseElement::new(99));
