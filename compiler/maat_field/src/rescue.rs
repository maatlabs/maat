...
//!
//! Lives in `maat_field` so the runtime (`maat_vm`), the trace recorder
//! (`maat_trace`), and the AIR (`maat_air`) all consume one source of
//! truth without inverting the crate-dependency graph.
