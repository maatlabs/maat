...
use crate::num::{self, NumToken, Suffix};
use crate::{Token, TokenKind};

/// A lexical analyzer for the Maat programming language.
///
/// The lexer converts raw source code into a stream of tokens, backed by a
...
impl<'a> Lexer<'a> {
    ...
    fn emit_token(&self, raw: RawToken) -> Token<'a> {
        let span = self.inner.span();
...
/// Internal token type produced by the logos compile-time DFA.
///
...
enum RawToken {
    ...
    #[regex(r"//[^\n]*", |_| logos::Skip, allow_greedy = true)]
...
fn skip_block_comment(lex: &mut logos::Lexer<'_, RawToken>) -> logos::Skip {
...
/// Tries to read and consume a type suffix from the lexer remainder.
///
...
fn try_read_suffix(lex: &mut logos::Lexer<'_, RawToken>) -> Option<Suffix> {
    let remainder = lex.remainder().as_bytes();
    let underscore = usize::from(remainder.first() == Some(&b'_'));
    let (suffix, len) = num::match_int_suffix(&remainder[underscore..])?;
    lex.bump(underscore + len);
    Some(suffix)
}

/// Lexes a radix-prefixed number (binary, octal, hex) with an optional type suffix.
fn lex_radix_number(lex: &mut logos::Lexer<'_, RawToken>) -> NumToken {
    let value_len = lex.slice().len() as u32;
    let kind = try_read_suffix(lex)
        .map(Suffix::to_token_kind)
...
fn lex_decimal_number(lex: &mut logos::Lexer<'_, RawToken>) -> NumToken {
    let value_len = lex.slice().len() as u32;
    let remainder = lex.remainder().as_bytes();
    ...
    if has_frac || has_exp {
        ...
        if has_frac {
            i += 1;
            while remainder.get(i).is_some_and(u8::is_ascii_digit) {
                i += 1;
            }
        }
        if remainder.get(i).is_some_and(|&b| b == b'e' || b == b'E') {
            i += 1;
            if remainder.get(i).is_some_and(|&b| b == b'+' || b == b'-') {
                i += 1;
            }
            while remainder.get(i).is_some_and(u8::is_ascii_digit) {
                i += 1;
