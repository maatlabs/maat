...
`maat_runtime` defines the `Value` enum--the common currency between the tree-walking interpreter/macro processor (`maat_eval`) and the bytecode VM (`maat_vm`). It also provides `Env` (a scope-chained variable store), the built-in function registry, integer wrapper types (`Integer`, `WideInt`) that enforce ZK-safe arithmetic (no floating-point, overflow-checked integer ops), and the `MemorySegmentManager` + `Relocatable` / `MaybeRelocatable` types that back the proof system's per-instance segmented heap. `Felt` from `maat_field` is re-exported here as a first-class `Value` variant.
