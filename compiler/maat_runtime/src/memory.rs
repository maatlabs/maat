...
mod tests {
    ...
    fn with_reserved_segments_preallocates_program_execution_and_public_io() {
        ...
        assert_eq!(mgr.num_segments(), 4);
        // All reserved segments start empty.
        assert_eq!(mgr.compute_sizes().unwrap(), vec![0, 0, 0, 0]);
    ...
    #[test]
    fn first_user_segment_after_reserved_init_is_id_four() {
        let mut mgr = MemorySegmentManager::with_reserved_segments();
        let first_user = mgr.add().unwrap();
        assert_eq!(first_user, Relocatable::new(4, 0));
        let second_user = mgr.add().unwrap();
        assert_eq!(second_user, Relocatable::new(5, 0));
    }
}
