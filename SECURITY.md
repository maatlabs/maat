...
Each attacker class below carries a **status**, so this document reads as a living ledger instead of a flat catalog of claims:

- **Mitigated@\<release\>** --- an active constraint, limit, or check defends the vector, guarded by a regression test, property test, or fuzz target. The release stamp records when the defense first shipped or was last reconciled.
- **Accepted-risk** --- the vector is understood and consciously left undefended because the current execution model (single-user, no deployed verifiers) puts it out of scope. Revisited if that model changes.
- **Open** --- a defense is designed but gated on a concrete consumer that makes it load-bearing. The gap is real, but no shipping example or test currently exercises it.

This ledger is part of the release checklist: every release re-checks each status against the shipped AIR before the version is tagged.
...
| Truncated or short proof                  | 36-byte minimum header parse rejects on insufficient bytes; magic `b"MATP"` and version `u16` (currently 6) validated before payload decode                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `maat_prover/src/gadgets.rs`                                                          |
...
| Uniform heap-write value forgery          | A published `HeapWrite` value and its co-committed public-output cell can be forged *together* so the memory-permutation accumulator still closes (the publish write has no read-back). The value pin `sel_heap_write * (mem_val - s0) = 0` binds every `HeapWrite` row's memory value to the stack top that produced it, so a forged publish diverges from the honest `s0` and fails the degree-2 constraint. `Opcode::ArenaFinalize` (which writes a fabricated segment marker, not `s0`) is split onto its own `SEL_ARENA_FINALIZE` selector so the pin gates on a now-pure `sel_heap_write`                                                                                                                                                          | `maat_trace/src/main_segment.rs` (`HEAP_WRITE_VAL_PIN`), `maat_trace/src/selector.rs` |
...
**Not yet mitigated (Accepted-risk):**
...
All 18 crates in the workspace enforce `#![forbid(unsafe_code)]`. The compiler toolchain contains zero `unsafe` blocks. Memory safety is guaranteed by the Rust type system and borrow checker.
