...
This document describes the trust boundaries, attacker model, and mitigations for the Maat compiler toolchain. It covers the current state as of v0.14.0 and will be updated as subsequent versions introduce new attack surfaces.
...
| Forged public output                      | `out[last] = public_output` boundary assertion is bound to the proof header; the verifier checks both the proof and the embedded inputs. For multi-cell public outputs (v0.14.0 `output_segment`), the public-memory accumulator's L2 endpoint binds the `(addr, val)` pairs of every published cell; the segment length is bound through Fiat-Shamir transcript absorption (`MaatPublicInputs::to_elements()`), so a verifier disagreeing with the prover on `output_segment.len()` derives different challenges and rejects | `maat_air/src/lib.rs`, `maat_air/src/aux_segment.rs`            |
...
**Soundness scope (v0.14.0):** the proof binds a primitive-typed program execution -- integers (`i8`..`i64`, `u8`..`u64`, `usize`), `bool`, `Felt`, fixed-size arrays `[T; N]` over primitive `T`, and user-defined functions over those types -- including bitwise operators (`BitAnd`/`BitOr`/`BitXor`/`Shl`/`Shr`) and unsigned ordering comparisons (`<`/`>`/`<=`/`>=` on `u8`/`u16`/`u32`/`usize`/`char`). The remaining surface composite types (`Vector<T>`, `Map<K,V>`, `Set<T>`, `str`, `struct`, `enum`, closures) execute correctly under `maat run` but their codegen is not yet migrated onto the segmented memory model -- `maat prove` will emit a proof on a composite-typed program, but the verifier rejects it because the trace omits constraint-satisfying rows for those operations. This is a completeness gap, not a soundness gap -- a verifier never accepts a tampered proof. Ordering for `u64`/`i64` and signed types requires a tighter range primitive and is deferred to a future release.
